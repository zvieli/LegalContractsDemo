// CommonJS diagnostic script to inspect arbitration-related state for a Rent contract
// Usage (PowerShell):
//  $env:RENT_ADDR = "0x..."; $env:REPORTER = "0x..."; npx hardhat run scripts/diagnoseArb.cjs --network localhost
// Or: npx hardhat run scripts/diagnoseArb.cjs --network localhost -- 0xRENT_ADDR 0xREPORTER
...
(async () => {
  try {
    ...
    try {
      const filter = rent.filters && rent.filters.DisputeResolved ? rent.filters.DisputeResolved() : (rent.filters && rent.filters['DisputeResolved'] ? rent.filters['DisputeResolved']() : null);
