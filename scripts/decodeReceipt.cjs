...
function tryHexToAscii(hex) {
  try {
    ...
    // only return printable-ish strings
    if (/\p{C}/u.test(str)) {
      // contains control chars — probably not readable
      return null;
    }
    return str;
  } catch (e) {
...
async function main() {
  ...
  if (!txHash) {
    console.error('Usage: node scripts/decodeReceipt.cjs <txHash>');
    process.exit(1);
  }

  // Connect directly to localhost node to avoid in-process hardhat provider ambiguity
  ...
  // Load the specific artifact ABI
  ...
  for (let i = 0; i < r.logs.length; i++) {
    ...
    if (!matched) {
      ...
      // Try to find ASCII inside the data (dynamic string fallback)
      if (looksLikeHexString(log.data)) {
        const ascii = tryHexToAscii(log.data);
        if (ascii) console.log('  data ascii:', ascii);
        else {
          // Try slicing off offset/length per ABI encoding to find embedded string
          try {
            // skip first 64 bytes (offset+length) and take next 256 bytes
