...
async function ensureSigned(ndaContract, signer, domain, types, value, opts={debug:false}){
...
async function ensureDeposit(ndaContract, signer, minDeposit, opts={debug:false}){
  const addr = await signer.getAddress();
  let current = 0n;
...
module.exports = { signTypedDataNode, ensureSigned, ensureDeposit };
