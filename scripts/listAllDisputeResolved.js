// ESM-compatible script to list DisputeResolved events
// Run with: npx hardhat run scripts/listAllDisputeResolved.js --network localhost
...
async function main() {
  ...
  for (const l of logs) {
    ...
    try {
      ...
      console.log('caseId:', parsed.args.caseId?.toString?.() || parsed.args[0]?.toString?.());
      console.log('approved:', parsed.args.approved);
      console.log('appliedAmount:', parsed.args.appliedAmount?.toString?.());
