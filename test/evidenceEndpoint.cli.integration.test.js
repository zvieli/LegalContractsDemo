...
describe('evidence endpoint CLI integration', function() {
  ...
  fs.writeFileSync(keyFile, identity.privateKey, { encoding: 'utf8' });
    ...
    // start endpoint as a child process with ADMIN_PUBLIC_KEY (spawn). The CLI and test are now robust to
    // library warnings on stderr because waitForLine listens on both stdout and stderr.
    ...
    // sanitize env: remove any ADMIN_PRIVATE_KEY* from inherited env to avoid accidental leakage
    ...
    try {
      const m1 = await waitForLine(child, /Evidence endpoint listening on http:\/\/127\.0\.0\.1:(\d+)/, 5000);
      const port = Number(m1[1]);
