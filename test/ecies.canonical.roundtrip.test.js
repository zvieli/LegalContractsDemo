...
describe('ECIES canonical roundtrip', function() {
  it.skip('encryptWithPublicKey -> decryptWithPrivateKey roundtrip returns identical symmetric key', async function() {
  ...
  const pubHex = Array.from(pubArr).map(b => b.toString(16).padStart(2, '0')).join('');
    ...
    // Skipped: ECIES deprecated in current version
  });
});
