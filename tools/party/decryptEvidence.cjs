#!/usr/bin/env node
// tools/party/decryptEvidence.cjs
// Usage: node tools/party/decryptEvidence.cjs <digest-or-filename> --privkey <0x...> [--address 0x...]
// Decrypts envelope for a party using their private key. Optionally provide address to match recipient.
...
async function main() {
  ...
  if (args.length < 3) {
    console.error('Usage: node tools/party/decryptEvidence.cjs <digest-or-filename> --privkey <0x...> [--address 0x...]');
    process.exit(1);
  }
  ...
  try {
    ...
    try {
      ...
      const encryptedBuffer = {
        ...
        // decrypt returns the original bytes; endpoint encrypted the hex string, so decode as utf8
