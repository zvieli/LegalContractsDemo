...
const STORAGE_DIR = path.resolve(process.cwd(), 'evidence_storage');
...
for (const f of files) {
  ...
  try {
    ...
    try {
      const out = child.spawnSync('node', [path.join(process.cwd(), 'tools', 'admin', 'decryptEvidence.js'), p], { encoding: 'utf8', timeout: 20000 });
    ...
    } catch (e) { report.failed.push({ file: f, error: e && e.message ? e.message : String(e) }); }
  } catch (e) { report.failed.push({ file: f, error: e && e.message ? e.message : String(e) }); }
}
